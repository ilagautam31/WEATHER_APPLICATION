...
const WEATHER_API_ENDPOINT='https://api.openweathermap.org/data/2.5/weather?units=metric&appid=4bf7b9e5e495b3ad6f7085ec6ccf3300&q=';
const WEATHER_DATA_ENDPOINT='https://api.openweathermap.org/data/2.5/onecall?appid=4bf7b9e5e495b3ad6f7085ec6ccf3300&exclude=minutely&units=metric&';
const FORECAST_API_ENDPOINT = 'https://api.openweathermap.org/data/2.5/forecast?appid=4bf7b9e5e495b3ad6f7085ec6ccf3300&units=metric&q=';
...
date.innerHTML = `${weekday}, ${fullDate} - ${time}`;
  ...
  .then(forecastData => {
    ...
    Object.keys(dayMap).slice(0, 5).forEach(dateKey => {
      const entries = dayMap[dateKey];
      const dateObj = new Date(entries[0].dt * 1000);

      const day = dateObj.toLocaleDateString(undefined, { weekday: 'long' }); // e.g., "Wednesday"
      const date = dateObj.toLocaleDateString(undefined, { day: 'numeric', month: 'short' }); // e.g., "9 Jul"
